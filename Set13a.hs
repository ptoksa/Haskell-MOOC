...
split word = do
  let string_words = words word
  if head string_words == word then Nothing else return (head string_words, string_words !! 1)
...
checkCapitals (for, sur) = intepret (check for >> check sur)
  where
    check x = if isUpper (head x) then return x else Nothing
    intepret Nothing = Nothing
    intepret _ = return (for, sur)
------------------------------------------------------------------------------
...
winner scores player1 player2 = do
  ...
  where
    compare' x y = if x >= y then return player1 else return player2
------------------------------------------------------------------------------
...
balance accountName = BankOp helper
  where
    helper (Bank b) = (Map.findWithDefault 0 accountName b, Bank b)
------------------------------------------------------------------------------
...
paren c = do
  val <- get
  if val < 0 then put val else classify c val
  where
    classify '(' val = modify (+ 1)
    classify _ val = put (val - 1)
...
count x = do
  list <- get
  case lookup x list of
    Nothing -> put (list ++ [(x, 1)])
