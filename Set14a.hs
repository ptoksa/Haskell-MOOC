...
greetText name
  | T.length name > 15 = T.pack "Hello, " <> T.take 15 name <> T.pack "...!"
  | otherwise = T.pack "Hello, " <> name <> T.pack "!"
------------------------------------------------------------------------------
...
--     ==> "WORD"
shout :: T.Text -> T.Text
shout x = go (T.words x) 0
  where
    go [] _ = mempty
    go [x] y = if even y then T.map toUpper x else x
    go (x : xs) y
      | even y = T.map toUpper x <> T.pack " " <> go xs 1
      | otherwise = x <> T.pack " " <> go xs 0
------------------------------------------------------------------------------
...
longestRepeat y
  ...
  | otherwise = go '-' y 0 0
  where
    go prev_char y c_max count = case T.uncons y of
      Nothing -> max c_max count
      Just (x, rest) -> if x == prev_char then go x rest (max c_max (count + 1)) (count + 1) else go x rest c_max 1
------------------------------------------------------------------------------
...
takeStrict a b = TL.toStrict (TL.take a b)
------------------------------------------------------------------------------
...
pingpong a = BL.cycle $ BL.fromStrict (a <> B.reverse a)
