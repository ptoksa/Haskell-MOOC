...
add a (Set xs) =
  if isMember
    then Set xs
    else Set $ quicksort (a : xs)
  where
    isMember = member a (Set xs)
quicksort :: Ord a => (Ord a) => [a] -> [a]
quicksort [] = []
...
data State = Start | AddingDry1 | AddedFlour | AddedSugar | Mixing | Baking | Error | Finished
  deriving (Eq, Show)

step :: State -> Event -> State
step Error _ = Error
step Finished _ = Finished
step Start e = if e == AddEggs then AddingDry1 else Error
step AddingDry1 e
  | e == AddFlour = AddedFlour
  | e == AddSugar = AddedSugar
  | otherwise = Error
step AddedFlour e = if e == AddSugar then Mixing else Error
step AddedSugar e = if e == AddFlour then Mixing else Error
step Mixing e = if e == Mix then Baking else Error
step Baking e = if e == Bake then Finished else Error
-- do not edit this
bake :: [Event] -> State
bake = go Start
  where
...
reverseNonEmpty (x :| []) = x :| []
reverseNonEmpty (x :| xs) = first :| rest
  where
...
instance (Eq a, Ord a) => Semigroup (Set a) where
  Set a <> Set b = Set (quicksort . nub $ a ++ b)
...
passwordAllowed pwd (MinimumLength a) = length pwd >= a
passwordAllowed pwd (ContainsSome a)
  | null pwd = False
  | otherwise = head pwd `elem` a || passwordAllowed (tail pwd) (ContainsSome a)
passwordAllowed pwd (DoesNotContain a)
  | null pwd = True
  | otherwise = head pwd `notElem` a && passwordAllowed (tail pwd) (DoesNotContain a)
