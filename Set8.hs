...
dotAndLine = Picture f
  where
    f (Coord x y)
      | x == 3 && y == 4 = white -- top corner is pink
...
combine f (Picture p1) (Picture p2) = Picture combined
  where
    combined (Coord x y) = f (p1 (Coord x y)) (p2 (Coord x y))
------------------------------------------------------------------------------
...
rectangle x0 y0 w h = Shape rect
  where
    rect (Coord x y) = xAxis x && yAxis y
    xAxis x = x >= x0 && x < (x0 + w)
    yAxis y = y >= y0 && y < (y0 + h)
------------------------------------------------------------------------------
...
union (Shape a) (Shape b) = Shape c
  where
    c (Coord x y) = a (Coord x y) || b (Coord x y)
...
paintSolid color (Shape s) (Picture base) = Picture new
  where
    new (Coord x y)
      | s (Coord x y) = color
...
paint (Picture pat) (Shape shape) (Picture base) = Picture new
  where
    new (Coord x y)
      | shape (Coord x y) = pat (Coord x y)
      | otherwise = base (Coord x y)
...
instance Transform Fill where
  apply (Fill color) (Picture p) = Picture new
    where
      new (Coord x y) = color
newtype Zoom = Zoom Int
  deriving (Show)
instance Transform Zoom where
  apply (Zoom z) (Picture p) = Picture new
    where
      new (Coord x y) = p $ Coord (x `div` z) (y `div` z)
...
instance Transform Blur where
  apply Blur p = Picture new
    where
      new (Coord x y) = avgCol $ neighbors (Coord x y) p
neighbors (Coord x y) (Picture p) = [p (Coord x y), p (Coord (x + 1) y), p (Coord (x -1) y), p (Coord x (y + 1)), p (Coord x (y -1))]
avgCol :: [Color] -> Color
avgCol [] = Color 0 0 0
avgCol (x : xs) = foldr addCol x xs `colDiv` length (x : xs)
addCol :: Color -> Color -> Color
addCol (Color a b c) (Color a2 b2 c2) = Color (a + a2) (b + b2) (c + c2)
colDiv :: Color -> Int -> Color
colDiv (Color a b c) x = Color (a `div` x) (b `div` x) (c `div` x)
------------------------------------------------------------------------------
